...
def load_visual_pretrain(model, path='/home/ducca/CMKD/C2KD/cremad_unpair/results/imagenetpre/resnet18-f37072fd.pth'):
    if not os.path.exists(path):
        print(f"Warning: Không tìm thấy file {path}, bỏ qua load pretrain.")
        return
    ...
    for k, v in state_dict.items():
        if k in model_dict:
            # Case 1: Khớp shape (đa số các layer)
            if v.shape == model_dict[k].shape:
                new_state_dict[k] = v
            # Case 2: Fix conv1 (ImageNet 3 kênh -> Input N kênh của bạn)
            elif k == 'conv1.weight':
                num_frames = model_dict[k].shape[1] // 3
                # Nhân bản weight theo số frame và chia trung bình để tránh nổ giá trị
...
def pre_train_models(stu_type, tea_type, loader, epochs, learning_rate, device, args, save_model=False):
    ...
    #     {'params': tea_model.parameters(), 'lr': 1e-4}, # Audio/Teacher
    #     {'params': stu_model.parameters(), 'lr': 2e-4}  # Image/Student (Tăng gấp đôi)
    # ], weight_decay=1e-4)
    # print(stu_type)
    # print(stu_model.__class__.__name__)
    # optimizer = torch.optim.SGD(list(tea_model.parameters()), lr=learning_rate, momentum=0.9)

    if tea_type == 0: load_visual_pretrain(tea_model) 
    if stu_type == 0: load_visual_pretrain(stu_model)
    # ------------------------------------------------------------------
