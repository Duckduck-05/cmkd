...
import pandas as pd
from utils.MMDDataset import CrisisMMDTextDataset, CrisisMMDHumanitarianTextDataset, CrisisMMDDataset_unpaired_random_text
import torch.nn as nn
from torch.optim import AdamW
import torch 
from model.Bert_based import SmallBertStudent, FeatureProjector
from model.MobileNet import MobileNetV2Student
from train_bert import get_pre_trained_teacher
...
def train():
    ...
    device = "cuda"
    student_model = MobileNetV2Student().to(device)
    print("Load unpaired data...")
    train_dataset = CrisisMMDDataset_unpaired_random_text(csv_file= "dataset/CrisisMMD_v2.0/crisismmd_datasplit_all/task_humanitarian_text_img_train.tsv",
                                              tokenizer= tokenizer,image_root="dataset/CrisisMMD_v2.0/",
                                              image_transform= image_transform,
                                              label_map= HUMANITARIAN_LABEL2ID)
    
    val_dataset = CrisisMMDDataset_unpaired_random_text(csv_file= "dataset/CrisisMMD_v2.0/crisismmd_datasplit_all/task_humanitarian_text_img_dev.tsv",
                                              tokenizer= tokenizer,image_root="dataset/CrisisMMD_v2.0/",
                                              image_transform= image_transform,
                                              label_map= HUMANITARIAN_LABEL2ID)
    
    test_dataset = CrisisMMDDataset_unpaired_random_text(csv_file= "dataset/CrisisMMD_v2.0/crisismmd_datasplit_all/task_humanitarian_text_img_test.tsv",
                                              tokenizer= tokenizer,image_root="dataset/CrisisMMD_v2.0/",
                                              image_transform= image_transform,
                                              label_map= HUMANITARIAN_LABEL2ID)
