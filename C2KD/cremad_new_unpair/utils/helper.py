...
def gen_data(data_dir, batch_size, num_workers, args):
    ...
    val_dataset = CremadDataset(args, mode='val')
...
# def pre_train_models(stu_type, tea_type, loader, epochs, learning_rate, device, args, save_model=False):
#     print("\n" + "="*40)
#     print("⚠️  DEBUG MODE: OVERFITTING 1 SINGLE BATCH  ⚠️")
#     print("="*40)
...
#     # Gom param để optimizer chạy cả 2
#     optimizer = torch.optim.SGD(list(tea_model.parameters()) + list(stu_model.parameters()), lr=learning_rate, momentum=0.9)

#     # --- BƯỚC 1: LẤY CỐ ĐỊNH 1 BATCH ---
#     # Không loop qua loader, chỉ lấy 1 cục data đầu tiên
#     data = next(iter(loader['train']))
#     img_inputs, aud_inputs, labels = data['image'].to(device), data['audio'].to(device), data['label'].to(device)

#     # --- BƯỚC 2: IN RA THÔNG SỐ ĐỂ BẮT LỖI ---
#     print(f"\n[DEBUG DATA STATS]")
#     print(f"Label Example: {labels[:5].cpu().numpy()}")
#     if stu_type == 0: # Student là Image
#         print(f"Student Input (Image): Shape={img_inputs.shape} | Min={img_inputs.min():.2f} | Max={img_inputs.max():.2f}")
#     else: # Student là Audio
#         print(f"Student Input (Audio): Shape={aud_inputs.shape} | Min={aud_inputs.min():.2f} | Max={aud_inputs.max():.2f}")
#     print("-" * 40)

#     # --- BƯỚC 3: VÒNG LẶP CƯỠNG ÉP (100 lần trên cùng 1 batch) ---
#     tea_model.train()
#     stu_model.train()

#     for step in range(101): # Chạy 100 bước là đủ biết kết quả
#         optimizer.zero_grad()

#         # Logic chọn input (đã sửa cho an toàn)
#         inp_stu = img_inputs if stu_type == 0 else aud_inputs
#         inp_tea = img_inputs if tea_type == 0 else aud_inputs

#         out_stu = stu_model(inp_stu)
#         out_tea = tea_model(inp_tea)

#         # Tính loss
#         loss1 = criterion(out_stu[0], labels) # Student Loss
#         loss2 = criterion(out_tea[0], labels) # Teacher Loss
#         loss = loss1 + loss2 
        
#         loss.backward()
#         optimizer.step()

#         # In kết quả mỗi 10 bước
#         if step % 10 == 0:
#             # Tính độ chính xác nhanh
#             acc_stu = (out_stu[0].argmax(dim=1) == labels).float().mean().item() * 100
#             acc_tea = (out_tea[0].argmax(dim=1) == labels).float().mean().item() * 100
            
#             print(f"Step {step:03d} | Loss: {loss.item():.4f} | Acc Stu: {acc_stu:.1f}% | Acc Tea: {acc_tea:.1f}%")

#     print("\n=== KẾT LUẬN ===")
#     if acc_stu > 90 and acc_tea > 90:
#         print("✅ SUCCESS: Model có khả năng học! (Vấn đề nằm ở Learning Rate hoặc Data Loader khi chạy thật)")
#     else:
#         print("❌ FAILED: Model không thể học nổi 1 batch. (Kiểm tra lại Input Shape, Normalization hoặc kiến trúc Model)")
    
#     # Return giả để code không lỗi
