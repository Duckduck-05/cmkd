...
def gen_data(data_dir, batch_size, num_workers, args):
    # data_dir hiện tại chưa dùng, nên truyền vào Dataset để code đỡ bị hard-code path
    # args.data_root = data_dir # (Optional: Gán đè nếu muốn linh hoạt path)
    ...
    train_dataloader = DataLoader(
        ...
        shuffle=True,           # Bắt buộc True để xáo trộn các batch
        num_workers=num_workers, 
        pin_memory=True,
        drop_last=True          # <--- NÊN THÊM: Bỏ batch cuối nếu không đủ size (tránh lỗi dimension)
    )

    test_dataloader = DataLoader(
        test_dataset, 
        batch_size=batch_size, 
        shuffle=False,          # Test/Val không cần shuffle
        num_workers=num_workers, 
        pin_memory=True
    )
